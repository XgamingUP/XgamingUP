...
#if defined(NDEBUG)
#error "XgamingUP cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uXGA) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight < 25) {
		nSubsidy = 8000 * COIN;
	} else if (nHeight < 500 && nHeight >= 25) {
        nSubsidy = 0.01 * COIN;
        ...
        nSubsidy = 50 * COIN;
	} else if (nHeight < 85000 && nHeight >= 75000) {
        nSubsidy = 70 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight < Params().LAST_POW_BLOCK() || blockValue == 0)
        return 0;
        ret = blockValue * 0.92; // 92% of block reward
...
{
    RenameThread("xgamingup-scriptch");
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.IsProofOfStake()) {
		...
		// Check stake input below minimum value
